...
pb <- txtProgressBar(min = 0,      # Minimum value of the progress bar
                     max = (length(batch)+1), # Maximum value of the progress bar
...
data_OD <- read.xlsx(filename, header = TRUE, sheetName = "Do600 raw")
data_scarlet <- read.xlsx(filename, header = TRUE, sheetName = "mScarlet raw")

#remove NA
data_OD <- data_OD[!is.na(data_OD[,3]),]
data_scarlet <- data_scarlet[!is.na(data_scarlet[,3]),]
...
meanblanck_OD <- as.data.frame(rowMeans(data_OD[grep("Blank", names(data_OD), ignore.case=T)]))
meanblanck_scarlet <- as.data.frame(rowMeans(data_scarlet[grep("Blank", names(data_scarlet), ignore.case=T)]))
...
for(x in 2:ncol(data_OD)){
 ...
 ODlaps <- (data_OD[(data_filter-3):(data_filter+3),x])
 scarletlaps <- (data_scarlet[(data_filter-3):(data_filter+3),x])
...
ggplot(plot, aes(y=growth_rate, x = ratio_meanScarletOD ))+
  geom_point()+
  xlab("Ratio Scarlet/OD")+
  ylab("Growth rate")+
  scale_y_continuous(limits=c(0, 2.25))+
  theme_bw() 

filename_save<- paste(Sys.Date(),"_export_",filename, sep = "")
...
filename_save<- paste(Sys.Date(),"_export_total.xlsx", sep = "")
write.xlsx(export_mixdata, file = filename_save ,sheetName="export", 
           col.names=TRUE, row.names=TRUE, append=FALSE)

setTxtProgressBar(pb, (length(batch)+1))  # Sets the final progress bar 
